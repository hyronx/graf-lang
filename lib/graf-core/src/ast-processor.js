...
class ASTProcessor {
  ...
  processBlock(astNode, position) {
    ...
    blockNodes.forEach((element, index, self) => {
      ...
      this.nodes.push(element, reportNode)
      this.links.push(
        element.linkWith(reportNode, LinkType.LEFT_TO_DEEPER_LEFT_TOP)
      )
  ...
  processAssign(astNode, position) {
    const variableNameNode = this.process(astNode.variable, position)
    ...
    const valueNode = this.process(astNode.value, {
      x: this.getNodeXPosition(position, 700),
      y: position.y,
        ...
        return new Node(
          ...
          position.column,
          {
            ...
            operation: new Operation(
              ...
              {
                result: new Result(astNode.value, "value"),
              }
            ),
            execStep: ++this.currentExecStep,
          }
